...
import {
  getAllSales as dbGetAllSales,
  getSalesPaginated as dbGetSalesPaginated,
  createSale as dbCreateSale,
  getAllProducts as dbGetAllProducts,
  getSalesCount as dbGetSalesCount
} from "../database";

export function getSalesList(page?: number, limit?: number, startDate?: string, endDate?: string) {
  if (page !== undefined || limit !== undefined) {
    return dbGetSalesPaginated(page || 1, limit || 50, startDate, endDate);
  }
  return dbGetAllSales();
}

export interface CreateSaleInputItem {
  productId: string;
  sku: string;
  quantity: number;
}

export interface CreateSaleData extends Record<string, unknown> {
  items: CreateSaleInputItem[];
  discountType?: string;
...
export function processNewSale(data: CreateSaleData): ProcessSaleResult {
  const { items, discountType, discountValue, taxRate, paymentMethod, amountPaid, staffId, staffName } = data;

  checkRequiredFields(data, ["items"]);
  ...
  const currentCount = dbGetSalesCount();
